...
class MerkleDamgardHash:
    ...
    def hash(self, message: str) -> str:
        ...
        # Обработка блоков
        for i in range(n):
            tmp = data[i * self.block_size : i * self.block_size + self.block_size]
            state = self.block_mixer.macro_compression(tmp, a + b + c + d + e)
            a, b, c, d, e = state
        c_block = CBlock()
        p1 = c_block.process([a, e], "16")
        c_block = CBlock()
        p2 = c_block.process([b, e], "16")
        c_block = CBlock()
        p3 = c_block.process([c, e], "16")
        c_block = CBlock()
        p4 = c_block.process([d, e], "16")
